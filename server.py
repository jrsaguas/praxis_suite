...
class PraxisRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_agent_graph_execute(self):
        ...
        try:
            ...
            depth_profile = mathematical_depth.MathematicalDepthProfile.from_request(
                profile_name,
    ...
    def handle_agent_graph_plan(self):
        try:
            ...
            profile = mathematical_depth.MathematicalDepthProfile.from_request(
    ...
    def do_GET(self):
        ...
        if path == '/api/system_config' or path == '/api/config':
            ...
            bits = parts[0].strip('/').split('/', 1)
            version_id = unquote(parts[1]).strip('/')
            if len(bits) != 2:
                self.send_error(400, 'Ruta de investigación inválida')
                return
            chat_id, folder = unquote(bits[0]), unquote(bits[1])
            try:
                navigation = investigation_store.navigate_versions(
                    chat_manager.CHATS_DIR, chat_id, folder, version_id
                )
                self.send_response(200)
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.end_headers()
                self.wfile.write(json.dumps({'status': 'ok', **navigation}, ensure_ascii=False).encode('utf-8'))
            except ValueError as e:
                self.send_error(404, str(e))
            except Exception as e:
                self.send_error(500, str(e))
